...
class PrivateRecipeAPITest(TestCase):
    ...
    def test_recipes_list_limited_to_user(self):
        """Test list of recipes is limited to authenticateduser."""
    ...
    def test_partial_update(self):
        ...
        # mandatory! if not  the values still be the same!
    ...
    def test_update_user_returns_error(self):
        ...
        # is testing that the user haven't changed.
        new_user = create_user(
            email='other22@example.com',
            password='password123'
        )
        recipe = create_recipe(user=self.user)
    ...
    def test_create_tag_on_update(self):
        ...
        # when changing a tag of a recipe, if it dosn't exists
        # is created.
        recipe = create_recipe(user=self.user)
        ...
        # since is a many to many field, there is no need to refresh from db
...
class ImageUploadTests(TestCase):
    ...
    def test_upload_image(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.jpg') as image_file:
            # temporary file.
            img = Image.new('RGB', (10, 10))  # creates a basic test image.
            img.save(image_file, format='JPEG')  # saves it.
            image_file.seek(0)  # goes back to the begining of the file.
            payload = {'image': image_file}
            res = self.client.post(url, payload, format='multipart')
            # always multipart form
